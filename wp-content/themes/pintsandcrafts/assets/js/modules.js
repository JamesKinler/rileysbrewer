...
(function($) {
    ...
    */
	...
	function edgtfRibbonWidget() {
		var ribbon = $('.edgtf-ribbon');

		if ( ribbon.length ) {
			ribbon.each(function(){
				var $this = $(this);
				$this.on('click', function(e){
					if ( $this.hasClass('edgtf-active') ) {
						if ( e.target.tagName !== 'A' ) {
							$this.removeClass('edgtf-active').find('.edgtf-ribbon-content').slideUp(650,'easeInOutElastic');
						}
					} else {
						$this.addClass('edgtf-active').find('.edgtf-ribbon-content').slideDown(650,'easeInOutElastic');
					}
				});
			});
		}
    ...
    /**
     * Init Ribbon Widget on appear
     */
    function edgtfInitRibbon() {
        var holder = $('.edgtf-ribbon');

        if(holder.length) {
            holder.each(function(){
                var thisHolder = $(this);

                thisHolder.addClass('edgtf-ribbon-appeared');
            });
        }
    }
                ...
                searchField.on('keyup paste', function() {
                    ...
                    keyPressTimeout = setTimeout( function() {
                        ...
                        if(searchTerm.length < 3) {
                            ...
                            var ajaxData = {
                                action: 'pintsandcrafts_edge_search_post_types',
                                term: searchTerm,
                                postType: postType
                            };
    ...
    function edgtfSectionTitleSeparator() {
        var holder = $('.edgtf-section-title-holder .edgtf-st-title-wrap svg');
        ...
        if(holder.length) {
            holder.each(function(){
                ...
                thisHolder.appear(function(){
                    thisHolder.addClass('edgtf-st-separator-appeared');
    ...
    **/
    ...
    function edgtfLikes() {
        $(document).on('click','.edgtf-like', function() {
            ...
            var dataToPass = {
                action: 'pintsandcrafts_edge_like',
                likes_id: id,
                type: type
            };
    ...
    function edgtfInitBlogListShortcodePagination(){
        ...
        var initMainPagFunctionality = function(thisHolder, pagedLink) {
            ...
            if(nextPage <= maxNumPages){
                ...
                var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'pintsandcrafts_edge_blog_shortcode_load_more');
	...
	*/
	...
	function edgtfResizePortfolioMasonryLayoutItems(size,container){
		if(container.find('.edgtf-ps-fixed-masonry').length) {
			var space_between_items = parseInt(container.find('.edgtf-ps-image').css('paddingLeft'), 10),
				newSize = size - 2 * space_between_items,
...
(function($) {
	...
	var edgtfButton = function() {
        ...
        /**
         * Initializes button hover border color, bordered type
         * @param button current button
         */
        var buttonHoverBordered = function(button) {
            if(typeof button.data('hover-item-bordered') !== 'undefined') {
                var changeBorderColor = function(event) {
                    event.data.button.find('.edgtf-btn-border-holder').css('border-color', event.data.color);
                };

                var originalBorderColor = button.find('.edgtf-btn-border-holder').css('borderTopColor'); //take one of the four sides
                var hoverBorderColor = button.data('hover-item-bordered');
...
(function($) {
	...
	function edgtfShowGoogleMap(){
		...
		if(googleMap.length){
			googleMap.each(function(){
				...
				var img;
				if(typeof element.data('image') !== 'undefined' && element.data('image') !== false) {
				...
				edgtfInitializeGoogleMap(snazzyMapStyle, snazzyMapCode, customMapStyle, colorOverlay, saturation, lightness, scrollWheel, zoom, holderId, mapHeight, pin,  map, geocoder, addresses, img, element);
			});
		}
	}
	 ...
	 **	Init Google Map
	 */
	function edgtfInitializeGoogleMap(snazzyMapStyle, snazzyMapCode, customMapStyle, color, saturation, lightness, wheel, zoom, holderId, height, pin,  map, geocoder, data, img, element){
		...
		for (index = 0; index < data.length; ++index) {
			edgtfInitializeGoogleAddress(data[index], pin, map, geocoder, img, element);
		}
	 ...
	 */
	function edgtfInitializeGoogleAddress(data, pin, map, geocoder, img, element){
		if (data === '') {
			return;
		}
		...
		geocoder.geocode( { 'address': data}, function(results, status) {
			var lat = results[0].geometry.location.lat(),
				...
				offsetMap = ( edgtf.windowWidth - element.outerWidth() ) < 50 ? 0.05 : 0.005;
			
			if (status === google.maps.GeocoderStatus.OK) {
				map.setCenter({lat: lat + offsetMap, lng: lng});
...
(function($) {
    ...
    function edgtfResizePortfolioItems(size,container){
        if(container.hasClass('edgtf-pl-images-fixed')) {
            var space_between_items = parseInt(container.find('article').css('padding-left'), 10),
	            newSize = size - 2 * space_between_items,
    ...
    function edgtfInitLoadMoreItemsPortfolioFilter($portfolioList, $filterValue, $filterClassName) {
        ...
        var	loadMoreDatta = edgtf.modules.common.getLoadMoreData(thisPortList),
            nextPage = loadMoreDatta.nextPage,
	        ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'pintsandcrafts_core_portfolio_ajax_load_more'),
                    ...
                    thisPortList.waitForImages(function () {
                        ...
                        if(portListHasArticles) {
                            setTimeout(function() {
                                ...
                                setTimeout(function() {
                                    thisPortListInner.isotope('layout').css('opacity', '1');
                                    edgtfInitPortfolioListAnimation();
	                                edgtf.modules.common.edgtfInitParallax();
                                }, 150);
                            }, 400);
	...
	function edgtfInitPortfolioPagination(){
		...
		var initMainPagFunctionality = function(thisPortList, pagedLink) {
			...
			if(nextPage <= maxNumPages || maxNumPages === 0){
				...
				var ajaxData = edgtf.modules.common.setLoadMoreAjaxData(loadMoreDatta, 'pintsandcrafts_core_portfolio_ajax_load_more');

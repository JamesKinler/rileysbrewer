...
/*
  Class: PintsAndCraftsPhpClassIconCollections
...
class PintsAndCraftsPhpClassIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new PintsAndCraftsPhpClassIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new PintsAndCraftsPhpClassIconsFontElegant("Font Elegant", "fe_icon"));
        $this->addIconCollection('ion_icons', new PintsAndCraftsPhpClassIonIcons("Ion Icons", "ion_icon"));
    }
    ...
    public function getIconsMetaBoxOrOption($attributes) {
	    ...
	    $options = array(
		    'font_awesome'      => esc_html__( 'Font Awesome', 'pintsandcrafts' ),
		    'font_elegant'      => esc_html__( 'Font Elegant', 'pintsandcrafts' ),
		    'ion_icons'         => esc_html__( 'Ion Icons', 'pintsandcrafts' )
	    );
	    ...
	    foreach ( $icon_collections as $collection_key => $collection_object ) {
		    ...
		    array_push( $edgt_icon_hide_values, '' ); //add empty value for icon switcher
		
		    $edgt_icon_pack_container = pintsandcrafts_edge_add_admin_container(
			    array(
				    'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'pintsandcrafts' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'pintsandcrafts' ),
	...
	public function getIconWidgetParamsArray() {
		...
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'pintsandcrafts' ),
			'options' => array(
				'font_awesome'      => esc_html__( 'Font Awesome', 'pintsandcrafts' ),
				'font_elegant'      => esc_html__( 'Font Elegant', 'pintsandcrafts' ),
				'ion_icons'         => esc_html__( 'Ion Icons', 'pintsandcrafts' )
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'pintsandcrafts' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			...
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'pintsandcrafts' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'pintsandcrafts' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'pintsandcrafts' ), $n ),
	...
	public function getIconCollectionIcons( iPintsAndCraftsPhpInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'pintsandcrafts' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'pintsandcrafts' ) ] = '';
...
if ( ! function_exists( 'pintsandcrafts_edge_activate_theme_icons' ) ) {
	function pintsandcrafts_edge_activate_theme_icons() {
		global $pintsandcrafts_php_global_IconCollections;
		$pintsandcrafts_php_global_IconCollections = PintsAndCraftsPhpClassIconCollections::get_instance();
	}
	
	add_action( 'after_setup_theme', 'pintsandcrafts_edge_activate_theme_icons' );
}
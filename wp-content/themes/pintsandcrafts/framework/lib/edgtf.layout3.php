...
/*
   Class: PintsAndCraftsPhpClassMultipleImages
   A class that initializes Edge Multiple Images
*/

class PintsAndCraftsPhpClassMultipleImages implements iPintsAndCraftsPhpInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$pintsandcrafts_php_global_Framework->edgtMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="edgtf-gallery-uploader">
								<a class="edgtf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'pintsandcrafts' ); ?></a>
								<a class="edgtf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'pintsandcrafts' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class PintsAndCraftsPhpClassTwitterFramework implements iPintsAndCraftsPhpInterfaceRender {
	public function render( $factory ) {
		$twitterApi = PintsAndCraftsTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'pintsandcrafts' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'pintsandcrafts' ) : esc_html__( 'Connect with Twitter', 'pintsandcrafts' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="edgtf-page-form-section" id="edgtf_enable_social_share">
			<div class="edgtf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'pintsandcrafts' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'pintsandcrafts' ); ?></p>
			</div>
			<div class="edgtf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class PintsAndCraftsPhpClassInstagramFramework implements iPintsAndCraftsPhpInterfaceRender {
	public function render( $factory ) {
		$instagram_api = PintsAndCraftsInstagramApi::getInstance();
		$message       = '';
		
		//if code wasn't saved to database
		if ( ! get_option( 'pintsandcrafts_instagram_code' ) ) {
			...
			if ( ! empty( $_GET['code'] ) ) {
				...
				$message = esc_html__( 'You have successfully connected with your Instagram account. If you have any issues fetching data from Instagram try reconnecting.', 'pintsandcrafts' );
		...
		$buttonText = $instagram_api->hasUserConnected() ? esc_html__( 'Re-connect with Instagram', 'pintsandcrafts' ) : esc_html__( 'Connect with Instagram', 'pintsandcrafts' );
		?>
		<?php if ( $message !== '' ) { ?>
			<div class="alert alert-success" style="margin-top: 20px;">
				<span><?php echo esc_html( $message ); ?></span>
		...
		<div class="edgtf-page-form-section" id="edgtf_enable_social_share">
			<div class="edgtf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'pintsandcrafts' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'pintsandcrafts' ); ?></p>
			</div>
			<div class="edgtf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class PintsAndCraftsPhpClassRepeater implements iPintsAndCraftsPhpInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ($this->table_layout) { ?>
				<div class="edgtf-repeater-table-heading">
					<div class="edgtf-repeater-fields-holder">
						<div class="edgtf-repeater-table-cell edgtf-repeater-sort"><?php esc_html_e( 'Order', 'pintsandcrafts' ) ?></div>
						...
						<div class="edgtf-repeater-table-cell edgtf-repeater-remove"><?php esc_html_e( 'Remove', 'pintsandcrafts' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="edgtf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="edgtf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = pintsandcrafts_edge_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class PintsAndCraftsPhpClassFieldAddress extends PintsAndCraftsPhpClassFieldType {
				...
				<div class="container-fluid">
					<div class="row">
						...
						<?php endif; ?>
							...
							<button id="find" class="btn btn-primary"><?php esc_html_e( 'Place the pin on the map', 'pintsandcrafts' ); ?></button>
...
class PintsAndCraftsPhpClassFieldIcon extends PintsAndCraftsPhpClassFieldType {
    public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
        $class = '';
        ...
        $icon_packs        = \PintsAndCraftsPhpClassIconCollections::get_instance()->getIconCollectionsEmpty();

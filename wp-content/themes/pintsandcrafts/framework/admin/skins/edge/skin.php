...
class EdgeSkin extends PintsAndCraftsPhpClassSkinAbstract {
    /**
     * Skin constructor. Hooks to pintsandcrafts_edge_admin_scripts_init and pintsandcrafts_edge_enqueue_admin_styles
     */
    public function __construct() {
        $this->skinName = 'edge';

        //hook to
        add_action('pintsandcrafts_edge_action_admin_scripts_init', array($this, 'registerStyles'));
        add_action('pintsandcrafts_edge_action_admin_scripts_init', array($this, 'registerScripts'));

        add_action('pintsandcrafts_edge_action_enqueue_admin_styles', array($this, 'enqueueStyles'));
        add_action('pintsandcrafts_edge_action_enqueue_admin_scripts', array($this, 'enqueueScripts'));

        add_action('pintsandcrafts_edge_action_enqueue_meta_box_styles', array($this, 'enqueueStyles'));
        add_action('pintsandcrafts_edge_action_enqueue_meta_box_scripts', array($this, 'enqueueScripts'));
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			pintsandcrafts_edge_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
            pintsandcrafts_edge_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = pintsandcrafts_edge_get_admin_tab();
        $active_page = $pintsandcrafts_php_global_Framework->edgtOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see PintsAndCraftsPhpClassSkinAbstract::loadTemplatePart()
     */
     ...
     *
     * @param PintsAndCraftsPhpClassAdminPage $page current page to load
     * @see PintsAndCraftsPhpClassSkinAbstract::loadTemplatePart()
     */

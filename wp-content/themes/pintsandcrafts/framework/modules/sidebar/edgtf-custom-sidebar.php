 ...
 */
if ( ! class_exists( 'PintsAndCraftsPhpClassSidebar' ) ) {
	class PintsAndCraftsPhpClassSidebar {
		var $sidebars = array();
		var $stored = "";
		...
		function __construct() {
			...
			$this->title  = esc_html__( 'Custom Widget Area', 'pintsandcrafts' );
		...
		function load_assets() {
			...
			wp_enqueue_script( 'edgtf_sidebar', EDGE_ROOT . '/framework/admin/assets/js/edgtf-ui/edgtf-sidebar.js' );
			wp_enqueue_style( 'edgtf_sidebar', EDGE_ROOT . '/framework/admin/assets/css/edgtf-sidebar.css' );
		}
		...
		function template_add_widget_field() {
			...
			echo "\n    <span class='input_wrap'><input type='text' value='' placeholder = '" . esc_html__('Enter Name of the new Widget Area', 'pintsandcrafts') . "' name='edgtf-add-widget' /></span>";
			echo "\n    <input class='button' type='submit' value='" . esc_html__('Add Widget Area', 'pintsandcrafts') . "' />";
			echo "\n    " . $nonce;
			echo "\n  </form>";
			echo "\n</script>\n";
		}
		...
		function register_custom_sidebars() {
			...
			$args = apply_filters( 'pintsandcrafts_edge_filter_custom_widget_args', $args );

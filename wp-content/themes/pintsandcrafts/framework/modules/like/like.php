...
class PintsAndCraftsPhpClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_pintsandcrafts_edge_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_pintsandcrafts_edge_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'pintsandcrafts' );
		
		if ( isset( $_COOKIE[ 'edgtf-like_' . $post->ID ] ) ) {
			$class = 'edgtf-like liked';
			$title = esc_html__( 'You already like this!', 'pintsandcrafts' );
		}
...
if ( ! function_exists( 'pintsandcrafts_edge_create_like' ) ) {
	function pintsandcrafts_edge_create_like() {
		PintsAndCraftsPhpClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'pintsandcrafts_edge_create_like' );
}
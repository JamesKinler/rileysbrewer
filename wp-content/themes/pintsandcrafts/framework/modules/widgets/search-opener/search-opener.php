...
class PintsAndCraftsPhpClassSearchOpener extends PintsAndCraftsPhpClassWidget {
	public function __construct() {
		parent::__construct(
			'edgtf_search_opener',
			esc_html__( 'Edge Search Opener', 'pintsandcrafts' ),
	...
	protected function setParams() {
		...
		$search_icon_params = array(
			array(
				...
				'description' => esc_html__( 'Define color for search icon', 'pintsandcrafts' )
			),
			array(
				'type'        => 'colorpicker',
				'name'        => 'search_icon_hover_color',
				'title'       => esc_html__( 'Icon Hover Color', 'pintsandcrafts' ),
				'description' => esc_html__( 'Define hover color for search icon', 'pintsandcrafts' )
			),
			array(
				'type'        => 'textfield',
			...
			array(
				'type'        => 'dropdown',
				'name'        => 'show_label',
				'title'       => esc_html__( 'Enable Search Icon Text', 'pintsandcrafts' ),
	...
	public function widget( $args, $instance ) {
		...
		$enable_search_icon_text	= pintsandcrafts_edge_options()->getOptionValue( 'enable_search_icon_text' );
		...
		<a <?php pintsandcrafts_edge_inline_attr( $instance['search_icon_hover_color'], 'data-hover-color' ); ?> <?php pintsandcrafts_edge_inline_style( $styles ); ?> <?php pintsandcrafts_edge_class_attribute( $search_icon_class_array ); ?> href="javascript:void(0)">
            <span class="edgtf-search-opener-wrapper">
                <?php if ( ( $search_icon_source == 'icon_pack' ) && isset( $search_icon_pack ) ) {
	                $pintsandcrafts_php_global_IconCollections->getSearchIcon( $search_icon_pack, false );
                } else if ( ( $search_icon_source == 'svg_path' ) && isset( $search_icon_svg_path ) ) {
                	print $search_icon_svg_path; 
                }?>
	            <?php if ( $show_search_text ) { ?>
		            <span class="edgtf-search-icon-text"><?php esc_html_e( 'Search', 'pintsandcrafts' ); ?></span>
	            <?php } ?>
            </span>
		</a>
	<?php }

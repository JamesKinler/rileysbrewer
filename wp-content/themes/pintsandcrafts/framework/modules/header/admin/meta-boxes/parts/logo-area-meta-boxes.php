...
if ( ! function_exists( 'pintsandcrafts_edge_get_hide_dep_for_header_logo_area_meta_boxes' ) ) {
	function pintsandcrafts_edge_get_hide_dep_for_header_logo_area_meta_boxes() {
		$hide_dep_options = apply_filters( 'pintsandcrafts_edge_filter_header_logo_area_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'pintsandcrafts_edge_get_hide_dep_for_header_logo_area_widgets_meta_boxes' ) ) {
	function pintsandcrafts_edge_get_hide_dep_for_header_logo_area_widgets_meta_boxes() {
		$hide_dep_options = apply_filters( 'pintsandcrafts_edge_filter_header_logo_area_widgets_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'pintsandcrafts_edge_header_logo_area_meta_options_map' ) ) {
	function pintsandcrafts_edge_header_logo_area_meta_options_map( $header_meta_box ) {
		$hide_dep_options = pintsandcrafts_edge_get_hide_dep_for_header_logo_area_meta_boxes();
		$hide_dep_widgets = pintsandcrafts_edge_get_hide_dep_for_header_logo_area_widgets_meta_boxes();
		...
		pintsandcrafts_edge_add_admin_section_title(
			array(
				...
				'title'  => esc_html__( 'Logo Area Style', 'pintsandcrafts' )
			)
		);
		...
		pintsandcrafts_edge_add_meta_box_field(
			array(
				...
				'type'          => 'yesno',
				'default_value' => 'no',
				'label'         => esc_html__( 'Disable Header Logo Area Widget', 'pintsandcrafts' ),
				'description'   => esc_html__( 'Enabling this option will hide widget area from the logo area', 'pintsandcrafts' ),
				'parent'        => $logo_area_container,
				'dependency' => array(
					'hide' => array(
		...
		if ( count( $pintsandcrafts_custom_sidebars ) > 0 ) {
			pintsandcrafts_edge_add_meta_box_field(
				array(
					...
					'type'        => 'selectblank',
					'label'       => esc_html__( 'Choose Custom Widget Area for Logo Area', 'pintsandcrafts' ),
					'description' => esc_html__( 'Choose custom widget area to display in header logo area"', 'pintsandcrafts' ),
					'parent'      => $logo_area_container,
		...
		pintsandcrafts_edge_add_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Set grid background transparency for logo area (0 = fully transparent, 1 = opaque)', 'pintsandcrafts' ),
				'parent'      => $logo_area_in_grid_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		pintsandcrafts_edge_add_meta_box_field(
			array(
				...
				'description'   => esc_html__( 'Set border on grid logo area', 'pintsandcrafts' ),
				'parent'        => $logo_area_in_grid_container,
				'default_value' => '',
				'options'       => pintsandcrafts_edge_get_yes_no_select_array()
			)
		...
		pintsandcrafts_edge_add_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a background color for logo area', 'pintsandcrafts' ),
				'parent'      => $logo_area_container
			)
		);
		...
		pintsandcrafts_edge_add_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a transparency for the logo area background color (0 = fully transparent, 1 = opaque)', 'pintsandcrafts' ),
				'parent'      => $logo_area_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		$logo_area_border_bottom_color_container = pintsandcrafts_edge_add_admin_container(
			array(
				'type'            => 'container',
		...
		do_action( 'pintsandcrafts_edge_action_header_logo_area_additional_meta_boxes_map', $logo_area_container );
	}
	
	add_action( 'pintsandcrafts_edge_action_header_logo_area_meta_boxes_map', 'pintsandcrafts_edge_header_logo_area_meta_options_map', 10, 1 );
}
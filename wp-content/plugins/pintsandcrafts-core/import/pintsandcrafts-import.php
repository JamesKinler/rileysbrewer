...
class PintsAndCraftsCoreImport {
	/**
	...
	/**
	 * @return PintsAndCraftsCoreImport
	 */
	...
	public function import_content( $file ) {
		ob_start();
		require_once( PINTSANDCRAFTS_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$pintsandcrafts_import = new WP_Import();
		set_time_limit( 0 );
		
		$pintsandcrafts_import->fetch_attachments = $this->attachments;
		$returned_value                  = $pintsandcrafts_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'pintsandcrafts-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'pintsandcrafts-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $pintsandcrafts_widget_id => $pintsandcrafts_widget_data ) {
			update_option( 'widget_' . $pintsandcrafts_widget_id, $pintsandcrafts_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'pintsandcrafts-core' );
	}
	
	public function import_sidebars_widgets( $file ) {
		$pintsandcrafts_sidebars = get_option( "sidebars_widgets" );
		unset( $pintsandcrafts_sidebars['array_version'] );
		$data = $this->file_options( $file );
		
		if ( is_array( $data['sidebars'] ) ) {
			$pintsandcrafts_sidebars = array_merge( (array) $pintsandcrafts_sidebars, (array) $data['sidebars'] );
			unset( $pintsandcrafts_sidebars['wp_inactive_widgets'] );
			$pintsandcrafts_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $pintsandcrafts_sidebars );
			$pintsandcrafts_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $pintsandcrafts_sidebars );
		}
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'pintsandcrafts-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'edgtf_options_pintsandcrafts', $options );
		$this->message = esc_html__( 'Options imported successfully', 'pintsandcrafts-core' );
	}
	
	public function import_menus( $file ) {
		global $wpdb;
		$pintsandcrafts_terms_table = $wpdb->prefix . "terms";
		$this->menus_data  = $this->file_options( $file );
		$menu_array        = array();
		
		foreach ( $this->menus_data as $registered_menu => $menu_slug ) {
			$term_rows = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM $pintsandcrafts_terms_table where slug=%s", $menu_slug ), ARRAY_A );
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $pintsandcrafts_page_option => $pintsandcrafts_page_id ) {
			update_option( $pintsandcrafts_page_option, $pintsandcrafts_page_id );
		}
	}
	...
	function edgtf_admin_import() {
		if ( pintsandcrafts_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				esc_html__( 'PintsAndCrafts Options - PintsAndCrafts Import', 'pintsandcrafts-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'pintsandcrafts-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
				'pintsandcrafts_edge_theme_menu' . $slug,                  // The ID used to bind submenu items to this menu
				array( $pintsandcrafts_php_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'pintsandcrafts_edge_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'pintsandcrafts_edge_enqueue_admin_styles' );
		}
	}
	...
	function edgtf_update_meta_fields_after_import($folder){
		...
		$images_field_array = pintsandcrafts_edge_meta_boxes()->getOptionsByType('image');
	...
	function edgtf_update_options_after_import($folder){
		...
		$global_options = get_option( 'edgtf_options_pintsandcrafts');
		$new_global_values = str_replace($demo_url, $url, $global_options);
		update_option( 'edgtf_options_pintsandcrafts', $new_global_values);
	...
	function edgtf_import_get_demo_url($folder) {
		...
		$demo_url	= 'http://' . $folder_new . '.edgt-themes.com/';

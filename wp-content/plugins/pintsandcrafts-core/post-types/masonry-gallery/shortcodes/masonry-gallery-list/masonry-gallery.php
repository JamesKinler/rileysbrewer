...
namespace PintsAndCraftsCore\CPT\Shortcodes\MasonryGallery;
...
class MasonryGallery implements Lib\ShortcodeInterface {
	...
	public function __construct() {
		...
		add_filter( 'vc_autocomplete_edgtf_masonry_gallery_category_callback', array( &$this, 'masonryGalleryCategoryAutocompleteSuggester', ), 10, 1 ); // Get suggestion(find). Must return an array
		
		//Masonry Gallery category render
		add_filter( 'vc_autocomplete_edgtf_masonry_gallery_category_render', array( &$this, 'masonryGalleryCategoryAutocompleteRender', ), 10, 1 ); // Get suggestion(find). Must return an array
	}
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Masonry Gallery', 'pintsandcrafts-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Items', 'pintsandcrafts-core' )
						),
						array(
							'type'        => 'dropdown',
						...
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
							'heading'     => esc_html__( 'Category', 'pintsandcrafts-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		if ( ! empty( $space_between_items ) ) {
			$holder_classes = 'edgtf-' . $space_between_items . '-space';
		}
		
		$query = new \WP_Query( $query_args );
		
		$html = '<div class="edgtf-masonry-gallery-holder edgtf-disable-bottom-space ' . esc_attr( $holder_classes ) . '">';
			$html .= '<div class="edgtf-mg-inner edgtf-outer-space">';
				$html .= '<div class="edgtf-mg-grid-sizer"></div>';
				$html .= '<div class="edgtf-mg-grid-gutter"></div>';
				
				if ( $query->have_posts() ) :
					while ( $query->have_posts() ) : $query->the_post();
						$itemID         = get_the_ID();
						$typeOption     = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_type', true );
						$title          = get_the_title( $itemID );
						$titleTagOption = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_title_tag', true );
						$itemSkin       = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_skin', true );
						$text           = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_text', true );
						$link           = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_link', true );
						$target         = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_link_target', true );
						$button_label   = get_post_meta( $itemID, 'edgtf_masonry_gallery_button_label', true );
						$button_type    = get_post_meta( $itemID, 'edgtf_masonry_gallery_button_type', true );
						...
						$params['item_button_type']     = ! empty( $button_type ) ? $button_type : 'solid';
						...
						$params['item_skin']            = ! empty( $itemSkin ) ? $itemSkin : '';
						
						$html .= pintsandcrafts_core_get_cpt_shortcode_module_template_part( 'masonry-gallery', 'masonry-gallery-list', 'masonry-gallery-' . $type . '-template', '', $params );
					
					endwhile;
				else:
					$html .= esc_html__( 'Sorry, no posts matched your criteria.', 'pintsandcrafts-core' );
				endif;
				wp_reset_postdata();
			$html .= '</div>';
		$html .= '</div>';
	...
	private function getItemClasses() {
		$classes = array( 'edgtf-mg-item' );
		
		$itemID          = get_the_ID();
		$type            = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_type', true );
		$image_size      = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_size', true );
        $link            = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_link', true );
        $shadow          = get_post_meta( $itemID, 'edgtf_masonry_gallery_item_shadow', true );
		
		if ( ! empty( $type ) ) {
			$classes[] = 'edgtf-mg-' . $type;
		}
		
		if ( ! empty( $image_size ) ) {
			$classes[] = 'edgtf-mg-' . $image_size;
		}

        if ( ! empty( $link ) ) {
            $classes[] = 'edgtf-mg-has-link';
        }

        if ( $shadow === 'yes' && $type !== 'simple' ) {
            $classes[] = 'edgtf-mg-has-shadow';
        }
	...
	public function getItemImage( $params ) {
		...
		$imageOption = get_post_meta( $id, 'edgtf_masonry_gallery_item_image', true );
		$item_image  = array();
		
		if ( ! empty( $imageOption ) ) {
			$image_url = $imageOption;
			$image_id  = pintsandcrafts_edge_get_attachment_id_from_url( $image_url );
	...
	public function masonryGalleryCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['masonry_gallery_category_title'] ) > 0 ) ? esc_html__( 'Category', 'pintsandcrafts-core' ) . ': ' . $value['masonry_gallery_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $masonry_gallery_category ) ) {
				...
				if ( ! empty( $masonry_gallery_category_title ) ) {
					$masonry_gallery_category_title_display = esc_html__( 'Category', 'pintsandcrafts-core' ) . ': ' . $masonry_gallery_category_title;
				}
